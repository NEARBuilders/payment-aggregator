...
import {
  getProviderAddressRequirementError,
  getProvidersAddressRequirementError,
} from "@/services/checkout/provider-address-requirements";
...
describe("provider address requirements", () => {
  it("requires a phone number for Lulu orders", () => {
    expect(getProviderAddressRequirementError("lulu", baseAddress)).toBe(
      "Phone number is required for delivery",
    );
  });

  it("does not require a phone number for manual orders", () => {
    expect(getProviderAddressRequirementError("manual", baseAddress)).toBeUndefined();
  });

  it("returns the first provider validation failure", () => {
    expect(getProvidersAddressRequirementError(["manual", "lulu"], baseAddress)).toEqual({
      provider: "lulu",
