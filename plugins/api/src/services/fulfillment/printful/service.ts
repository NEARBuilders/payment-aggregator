...
export class PrintfulService {
  ...
  browseCatalog(input: {
    limit?: number;
    offset?: number;
  }): Effect.Effect<BrowseCatalogOutput, FulfillmentError> {
    return Effect.tryPromise({
      try: async () => {
  ...
  getCatalogProduct(input: {
    id: string;
  }): Effect.Effect<CatalogProductDetailOutput, FulfillmentError> {
    return Effect.tryPromise({
      try: async () => {
  ...
  getCatalogProductVariants(input: {
    id: string;
  }): Effect.Effect<CatalogVariantsOutput, FulfillmentError> {
    return Effect.tryPromise({
      try: async () => {
        const providerRef = input.id.replace("printful-", "");
      ...
      try: async () => {
        ...
        return {
          price: primaryTechnique
            ? {
                cost: primaryTechnique.price,
                discountedCost: primaryTechnique.discountedPrice || primaryTechnique.price,
  ...
  }): Effect.Effect<GetPlacementsOutput, FulfillmentError> {
  ...
  ): Effect.Effect<GenerateMockupsOutput, FulfillmentError> {
    return Effect.gen(this, function* () {
      ...
      const payload = {
        format:
          input.format === "png" ? MockupTaskCreation.format.PNG : MockupTaskCreation.format.JPG,
        products: [
          {
      ...
      const response = yield* Effect.tryPromise({
        try: async () => {
          const result = await this.client.mockupGeneratorV2.createMockupGeneratorTasks(
            this.client.getStoreId(),
            payload,
          );
      ...
      try: async () => {
        ...
        if (task.status === MockupGeneratorTask.status.FAILED) {
          return {
            ...
            images: [],
            error: task.failure_reasons.map((e) => e.detail).join(", "),
          };
        }
  ...
  getMockupStyles(productId: number): Effect.Effect<{ styles: MockupStyleInfo[] }, Error> {
    return Effect.tryPromise({
      try: async () => {
        ...
        new Error(`Failed to get mockup styles: ${e instanceof Error ? e.message : String(e)}`),
    });
  }
  ...
  createOrder(input: CreateOrderInput): Effect.Effect<OrderResult, FulfillmentError> {
    return Effect.tryPromise({
      try: async () => {
        ...
        const catalogProductsByProductId = new Map<
          number,
          {
        ...
        if (itemsWithMissingTechnique.length > 0) {
          ...
          for (const productId of productIds) {
            if (!catalogProductsByProductId.has(productId)) {
              ...
              if (!catalogProduct) {
                console.warn(
                  `[PrintfulService.createOrder] Catalog product ${productId} not available — cannot resolve missing techniques`,
                );
              }
            }
          }
        ...
        const orderItems = input.items.map((item) => {
          ...
          const placements = (item.files || [])
            ...
            .map((df: any) => {
              ...
              if (!technique) {
                ...
                if (slot === "default") {
                  ...
                  technique =
                    catalogProduct?.placementTechniques?.[slot] ??
                    catalogProduct?.primaryPlacement?.technique;
                }
                if (technique) {
                  console.info(
                    `[PrintfulService.createOrder] Resolved technique "${technique}" for placement "${slot}" from catalog product ${config.catalogProductId}`,
                  );
                } else {
                  console.warn(
                    `[PrintfulService.createOrder] Could not resolve technique for placement "${slot}" on catalog product ${config.catalogProductId} — file will be skipped`,
                  );
                }
              }
            ...
            .filter(
              (p): p is { placement: string; technique: string; url: string } => !!p.technique,
            )
            .map((p) => ({
  ...
  quoteShipping(input: ShippingQuoteInput): Effect.Effect<ShippingQuoteOutput, FulfillmentError> {
    return Effect.gen(this, function* () {
      ...
      const result = yield* Effect.tryPromise({
        try: () =>
          this.client.calculateShippingRates({
            recipient: {
              country_code: input.recipient.countryCode,
  ...
  calculateTax(input: TaxQuoteInput): Effect.Effect<TaxQuoteOutput, FulfillmentError> {
    ...
    return Effect.gen(this, function* () {
      ...
      try {
        ...
        if (isQuoteMode) {
          console.warn(
            `[printful] Tax calculation skipped during quote after ${Date.now() - startedAt}ms: ${error instanceof Error ? error.message : String(error)}`,
          );
          return {
            required: false,
            rate: 0,
    ...
    for (const syncProduct of allSyncProducts) {
      ...
      try {
        ...
        if (catalogProductId) {
          ...
          if (!catalogProduct) {
            console.warn(
              `[PrintfulService.syncProducts] Catalog product ${catalogProductId} not available for "${sync_product.name}" — placement/technique data will be missing. Orders for these variants will resolve techniques at creation time.`,
            );
          }
        } else {
          console.warn(
            `[PrintfulService.syncProducts] No catalogProductId found for "${sync_product.name}" — placement/technique data will be missing.`,
          );
        }
        ...
        if (catalogProductId) {
          try {
            ...
            console.warn(
              `[PrintfulService.syncProducts] Failed to fetch pricing data for "${sync_product.name}": ${e instanceof Error ? e.message : String(e)}`,
            );
          }
        }
  ...
  private extractDesignFiles(
    ...
    if (!catalogPlacements) {
      console.warn(
        `[PrintfulService.extractDesignFiles] No catalog placement data for "${syncProductName ?? "unknown"}" — technique metadata will be missing. Orders for these variants will need to resolve techniques at creation time.`,
      );
    }

    const bySlot = new Map<
      string,
      { file: (typeof files)[number]; slot: string; technique: string | null; resolvedUrl: string }
    >();
    ...
    for (const f of files) {
      ...
      if (!technique) {
        console.warn(
          `[PrintfulService.extractDesignFiles] No technique resolved for file ${f.id} (type="${type}", slot="${slot}") on "${syncProductName ?? "unknown"}" — this will need resolution at order time`,
        );
      }
  ...
  private transformSyncProductToV2(
    ...
    const variants = syncVariants.map((v) => {
      ...
      const additionalPlacementCost = activePlacements
        ...
        .reduce((sum, p) => {
          const placementPrice =
            variantPrice?.placements.find((pl) => pl.placement === p.slot)?.price ??
      ...
      const fulfillmentConfig: FulfillmentConfig = {
        ...
        providerConfig: {
          ...
          ...(Object.keys(allPlacementPricing).length > 0
            ? { placementPricing: allPlacementPricing }
            : {}),
    ...
    if (catalogProduct) {
      ...
      if (catalogProduct.description)
        providerDetails.gsm = this.extractGsm(catalogProduct.description);
    }

    const basePrice =
      syncVariants.length > 0
        ? Math.min(
            ...syncVariants.map((v) => (v.retail_price ? parseFloat(v.retail_price) : Infinity)),
          )
        : 0;
  ...
  getOrderShipments(orderId: string) {
    return Effect.tryPromise({
      try: async () => {
        ...
        new Error(
          `Failed to get Printful shipments: ${e instanceof Error ? e.message : String(e)}`,
        ),
    });
  }
  ...
  configureWebhooks(params: {
    ...
    return Effect.tryPromise({
      try: async () => {
        ...
        new Error(
          `Failed to configure Printful webhooks: ${e instanceof Error ? e.message : String(e)}`,
        ),
    });
  }
  ...
  disableWebhooks() {
    return Effect.tryPromise({
      try: async () => {
        ...
        new Error(
          `Failed to disable Printful webhooks: ${e instanceof Error ? e.message : String(e)}`,
        ),
    });
  }
  ...
  getWebhookConfig() {
    return Effect.tryPromise({
      try: async () => {
        ...
        new Error(
          `Failed to get Printful webhook config: ${e instanceof Error ? e.message : String(e)}`,
        ),
    });
  }
    ...
    return Effect.gen(this, function* () {
      ...
      if (styleIds.length === 0) {
        ...
        const relevant = styles.filter(
          (s) => s.category === "Flat" || s.category === "Lifestyle" || s.category === "Men's",
        );
      ...
      for (const variantId of params.variantIds) {
        try {
          const files = params.designFiles
            .map((df) => {
              let technique: string | null = null;
  ...
  async handleCatalogPriceChange(
    ...
    if (!product) {
      console.log(
        `[Printful] No local product found for catalog product ${catalogProductId}, skipping price update`,
      );
      return;
    }

    console.log(
      `[Printful] Catalog price changed for product ${product.id} (catalog: ${catalogProductId}), re-syncing`,
    );
    ...
    try {
      ...
      if (catalogProductId) {
        try {
          ...
          console.warn(
            `[Printful] Failed to fetch pricing data for catalog_price_changed: ${e instanceof Error ? e.message : String(e)}`,
          );
        }
      }
      ...
      await Effect.runPromise(productStore.upsert(transformed, new Date()));
    } catch (error) {
      console.error(
        `[Printful] Failed to re-sync product after catalog price change: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
