...
import type { LuluService } from "./lulu/service";
...
> {
  ...
  return Effect.gen(function* () {
    ...
    if (update.newStatus) {
      currentOrder = yield* orderStore.updateStatus(order.id, update.newStatus, actor, eventType, {
        eventType,
        ...metadata,
      });
    }
    ...
    if (update.shouldRetryConfirmation) {
      ...
      if (Object.keys(draftOrderIds).length > 0) {
          ...
          try {
            yield* Effect.tryPromise({
              try: () => provider.client.confirmOrder({ id: draftId as string }),
                ...
                new Error(`Failed to confirm: ${e instanceof Error ? e.message : String(e)}`),
        ...
        allProviderConfirmationsSucceeded = Object.values(confirmationResults).every(
    ...
    if (update.newTracking) {
      currentOrder = yield* orderStore.updateTracking(order.id, update.newTracking, actor, {
        eventType,
        ...metadata,
      });
    }
...
export function processLuluWebhookEffect(options: {
  ...
  return Effect.gen(function* () {
    ...
    if (eventType === "PRINT_JOB_STATUS_CHANGED") {
      const luluStatus =
        typeof data.status === "string" ? data.status : data.status?.name || "CREATED";
      ...
      if (luluStatus === "REJECTED" || luluStatus === "ERROR") {
        console.error(
          `[Lulu Webhook] Print job ${typeof data.status === "string" ? data.status : data.status?.name} for order ${order.id}:`,
          {
            errors: errorDetails,
            rawData: data,
          },
    ...
    let currentOrder = order;
    if (newStatus) {
      currentOrder = yield* orderStore.updateStatus(order.id, newStatus, actor, eventType, {
        eventType,
        errorDetails,
        ...metadata,
      });
    ...
    if (trackingInfo) {
      currentOrder = yield* orderStore.updateTracking(order.id, trackingInfo, actor, {
        eventType,
        ...metadata,
      });
    }
