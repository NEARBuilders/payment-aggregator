...
export class R2Client {
  ...
  async generatePresignedPutUrl(
    ...
    const signed = await this.client.sign(url, {
      ...
      aws: { signQuery: true, allHeaders: true, expires: expiresIn } as Parameters<
        typeof this.client.sign
      >[1] extends { aws?: infer A }
        ? A
        : never,
    });
    return signed.url;
  ...
  async generatePresignedGetUrl(key: string, expiresIn = 3600): Promise<string> {
    const url = `${this.endpoint}/${this.bucket}/${key}`;
    const signed = await this.client.sign(url, {
      aws: { signQuery: true, expires: expiresIn } as Parameters<
        typeof this.client.sign
      >[1] extends { aws?: infer A }
        ? A
        : never,
