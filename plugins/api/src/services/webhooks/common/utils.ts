...
export async function verifyHMACSignature(
  ...
  const expected = crypto.createHmac("sha256", secretBuffer).update(body).digest("hex");
...
export function parseJSON<T = unknown>(body: string): Effect.Effect<T, Error> {
  return Effect.try({
    try: () => JSON.parse(body) as T,
      ...
      new Error(`Failed to parse JSON: ${error instanceof Error ? error.message : String(error)}`),
  });
}
