...
export const cleanupAbandonedDrafts = (runtime: MarketplaceRuntime, maxAgeHours = 24) =>
  Effect.gen(function* () {
    ...
    for (const order of abandonedOrders) {
      ...
      if (!order.draftOrderIds || Object.keys(order.draftOrderIds).length === 0) {
        console.log(
          `[CleanupJob] Order ${order.id} has no draft orders to cancel, marking as cancelled`,
        );
      ...
      for (const [providerName, draftId] of Object.entries(order.draftOrderIds)) {
        ...
        if (cancelResult.success) {
          console.log(
            `[CleanupJob] Successfully cancelled draft ${draftId} at ${providerName} for order ${order.id}`,
          );
        } else {
          const errorMessage = "error" in cancelResult ? cancelResult.error : "Unknown error";
          console.error(
            `[CleanupJob] Failed to cancel draft ${draftId} at ${providerName} for order ${order.id}:`,
            errorMessage,
          );
          errors.push({
            orderId: order.id,
